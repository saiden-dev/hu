...
mod tests {
    ...
    fn sprint_clone() {
        ...
        assert_eq!(cloned.state, sprint.state);
    }

    #[test]
    fn sprint_without_dates() {
        let sprint = Sprint {
            id: 200,
    ...
    fn board_clone() {
        ...
        assert_eq!(cloned.id, board.id);
        assert_eq!(cloned.name, board.name);
        assert_eq!(cloned.board_type, board.board_type);
    }

    #[test]
    fn board_debug_format() {
        let board = Board {
            id: 1,
            name: "B".to_string(),
    ...
    fn board_serialize() {
        ...
        assert!(json.contains("123"));
        assert!(json.contains("Dev Board"));
    }

    #[test]
    fn board_deserialize() {
        let json = r#"{"id": 456, "name": "Test Board", "board_type": "kanban"}"#;
        let board: Board = serde_json::from_str(json).unwrap();
        assert_eq!(board.id, 456);
        assert_eq!(board.name, "Test Board");
        assert_eq!(board.board_type, "kanban");
    }
